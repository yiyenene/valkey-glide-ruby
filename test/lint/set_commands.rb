...
module Lint
  module SetCommands
    ...
    def test_variadic_srem
      ...
      assert_equal 1, r.srem("foo", %w[s1 aaa])
      assert_equal 0, r.srem("foo", %w[bbb ccc ddd])
      assert_equal 1, r.srem("foo", %w[eee s3])
    ...
    def test_spop
      ...
      assert %w[s1 s2].include?(r.spop("foo"))
      assert %w[s1 s2].include?(r.spop("foo"))
    ...
    def test_spop_with_positive_count
      ...
      assert !(%w[s1 s2 s3 s4] & pops).empty?
    ...
    def test_srandmember
      ...
      4.times do
        assert %w[s1 s2].include?(r.srandmember("foo"))
      end
    ...
    def test_srandmember_with_positive_count
      ...
      4.times do
        assert !(%w[s1 s2 s3 s4] & r.srandmember("foo", 3)).empty?
    ...
    def test_srandmember_with_negative_count
      ...
      4.times do
        assert !(%w[s1 s2 s3 s4] & r.srandmember("foo", -6)).empty?
